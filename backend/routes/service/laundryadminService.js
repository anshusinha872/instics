...
let laundrystatusupdate = async (req) => {
	console.log(req);
	try {
		// console.log(req);
...
let paymenthistory=async(data)=>{
	try {
		...
		const response = await new Promise((resolve, reject) => {
			const query = 'select * from laundryOrderRequest WHERE LEFT (laundryOrderRequestTxnId, 1) =? && status=?;;';
...
let totalpayment=async(data)=>{
	try {
		...
		const response = await new Promise((resolve, reject) => {
			const query = 'select sum(CAST(finalAmountAfterDiscount AS UNSIGNED)) AS totalpayment from laundryOrderRequest WHERE LEFT (laundryOrderRequestTxnId, 1) =? && status=?;';
...
let successpayment=async(req)=>{
	try {
		...
		let printPaymentReport = [];
		let laundryPaymentReport = [];
		if(response.length>0){
			for(let i = 0; i < response.length; i++){
				console.log(response[i].client_txn_id);
				// laundryPaymentReport
				const laundryPaymentReportResponse = await new Promise((resolve, reject) => {
					const query = 'select * from laundryOrderRequest where paymentRequestTxnId = ?';
					connection.query(query,[response[i].client_txn_id],(err,results) =>{
						if (err) reject(new Error(err.message));
							resolve(results);
					});
				});
				if(laundryPaymentReportResponse.length>0){
					for(let i=0;i<laundryPaymentReportResponse.length;i++){
						laundryPaymentReport.push(laundryPaymentReportResponse[i]);
					}
				}
				// printPaymentReport
				const printPaymentReportResponse = await new Promise((resolve, reject) => {
					const query = 'select * from pdfOrderRequest where paymentRequestTxnId = ?';
					connection.query(query,[response[i].client_txn_id],(err,results) =>{
						if (err) reject(new Error(err.message));
							resolve(results);
					});
				});
				if(printPaymentReportResponse.length>0){
					for(let i=0;i<laundryPaymentReportResponse.length;i++){
						printPaymentReport.push(laundryPaymentReportResponse[i]);
					}
				}
			}
		}
		returnData.push(laundryPaymentReport);
		returnData.push(printPaymentReport);
		return returnData;
	} catch (err) {
		console.log(err);
		return resultdb(500, err);
